...
<div class="w-full p-4 flex flex-col">
    ...
    <form on:submit|preventDefault={submit} class="flex flex-col items-center w-full">
        ...
        <div class="form-control w-full">
            ...
            <InputPassword bind:value={$form.password_confirmation} type="password" name="password_confirmation" placeholder="Password Confirmation" class="w-full input input-bordered {$form.errors.password ? 'input-error' : ''}" required />
            {#if $form.errors.password}
                <span class="label-text-alt text-error text-left mt-1">{$form.errors.password_confirmation}</span>
            {/if}
        </div>
